...
public class RecipesWallumTeleport implements IRecipe {
	...
	public ItemStack getCraftingResult(InventoryCrafting arg0) {
		...
		for(int i = 0; i < arg0.getSizeInventory(); i++) {
			...
			if(s==null) continue;
			if(s.getItem().equals(ItemsAndrew.wallumTeleport)) {
				if(wallum!=null) {return null;}
				wallum = s;
			}
			else if(OreDictionary.itemMatches(new ItemStack(Items.ender_pearl),s,false)) {
				add+=3;
			}
			else {
				return null;
