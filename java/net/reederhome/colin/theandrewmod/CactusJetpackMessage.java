...
public class CactusJetpackMessage implements IMessage {
	...
	public static class Handler implements IMessageHandler<CactusJetpackMessage,IMessage> {
		...
		public IMessage onMessage(CactusJetpackMessage arg0, MessageContext arg1) {
			...
			if(e.getCurrentArmor(2)!=null && e.getCurrentArmor(2).getItem().equals(ItemsAndrew.cactusGunJetpack) && e.inventory.hasItem(Item.getItemFromBlock(Blocks.cactus))) {
				...
				if(Math.random()<0.3) {
					e.inventory.consumeInventoryItem(Item.getItemFromBlock(Blocks.cactus));
				}
			}
			return null;
		}
